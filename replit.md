...
- cc.fwafarm.com renders the player name as plain text right after a `<b>Name: </b>` label — but for players who have changed their in-game name, it uses `<b>Names: </b>` (plural) instead, followed by a `<span>` "This player has changed their name!" notice. The parser must handle both labels and read the raw text between the label and the next `<br>`/tag, not a single-word regex, since names can contain spaces (e.g. "Sub - zero").
- The real ban explanation on cc.fwafarm.com lives in the "Member Notes" table (a `<td class="bigted">` cell in the row after the header), not near the "BANNED!" marker itself. Text scraped from around the word "ban" on the page is unreliable garbage (mixes in unrelated boilerplate) — always prefer the structured Member Notes cell, and omit the reason entirely if that cell isn't found rather than falling back to a scraped snippet.
