...
<<<<<<< HEAD
- **ScrapingAnt does not bypass this site either (cc.fwafarm.com, 2026-07-05):** free-tier `browser=true` requests, with and without `proxy_type=residential`/`proxy_country=US`/explicit `wait`, either returned a 423 "our browser was detected" error or a 200 that was still the raw Cloudflare "Just a moment..." interstitial. When stacking multiple scraping-API vendors as fallbacks for the *same* hard Cloudflare target, don't assume vendor diversity alone fixes it — some vendors' free/base tiers are equally detectable. Verify each vendor actually returns real content for the specific target before relying on it, and document in the fallback code/README when a "backup" is best-effort only, not a proven bypass.
=======
